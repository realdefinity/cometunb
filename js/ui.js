...
function showModal(title, desc, onConfirm) {
    ...
    newBtn.onclick = () => { overlay.classList.remove('visible'); onConfirm(); };
    document.getElementById('btn-cancel').onclick = () => overlay.classList.remove('visible');
    overlay.classList.add('visible');
}

const THEMES = [
    { id: 'classic', name: 'Classic' }, { id: 'midnight', name: 'Midnight' },
    { id: 'amoled', name: 'OLED Black' }, { id: 'cyberpunk', name: 'Cyberpunk' },
    { id: 'paper', name: 'Paper' }, { id: 'ocean', name: 'Ocean' },
    { id: 'forest', name: 'Forest' }, { id: 'sunset', name: 'Sunset' },
    { id: 'cloud', name: 'Cloud' }, { id: 'matrix', name: 'Matrix' },
    { id: 'royal', name: 'Royal' }, { id: 'coffee', name: 'Coffee' },
    { id: 'lavender', name: 'Lavender' }, { id: 'mint', name: 'Mint' },
...
function renderThemes() {
    ...
    if(!p) return;
    p.innerHTML = THEMES.map(t => `<div class="t-opt ${document.documentElement.getAttribute('data-theme') === t.id ? 'active' : ''}" onclick="setTheme('${t.id}')">${t.name}</div>`).join('');
}
...
function hidePreview() { const p = document.getElementById('link-preview'); if(p) p.classList.remove('visible'); }
...
function renderGalaxy(history) {
    ...
    ctx.scale(dpr, dpr);
    const nodes = [];
    const width = rect.width; const height = rect.height; const padding = 60;
    ...
    history.forEach((title, i) => {
        ...
        nodes.push({ x: x, y: y, title: title, baseR: i === 0 || i === history.length - 1 ? 6 : 3, hoverR: 0 });
    });
    ...
    function draw() {
        ...
        if (nodes.length > 1) {
            ...
            for (let i = 0; i < nodes.length - 1; i++) {
                const p0 = nodes[i]; const p1 = nodes[i + 1];
        ...
        nodes.forEach((n) => {
            ...
            if (n.hoverR < 0) n.hoverR = 0; if (n.hoverR > 4) n.hoverR = 4;
            if (isHover) activeNode = n;
            ctx.beginPath();
            ctx.arc(n.x, n.y, n.baseR + n.hoverR, 0, Math.PI * 2);
            ctx.fillStyle = "white"; ctx.shadowBlur = 10 + (n.hoverR * 5); ctx.shadowColor = "white";
...
function getStats() {
    ...
    const defaultStats = {
        gamesPlayed: 0, wins: 0, currentStreak: 0, bestStreak: 0,
        totalClicks: 0, totalTimeSeconds: 0, fastestRun: null, themeUsage: {}
    };
...
function saveGameStats(isWin, timeInSeconds, clicks) {
    ...
    if (isWin) {
        ...
        if (s.fastestRun === null || timeInSeconds < s.fastestRun) s.fastestRun = timeInSeconds;
...
function showStats() {
    ...
    let favTheme = "None"; let maxCount = 0;
    for (const [theme, count] of Object.entries(s.themeUsage)) {
        if (count > maxCount) { maxCount = count; favTheme = theme; }
    }
...
function closeStats() { const s = document.getElementById('stats-screen'); if(s) s.classList.add('hidden'); }
...
function animateText(elementId, finalText) {
    ...
    const interval = setInterval(() => {
        ...
        if(iterations >= finalText.length) { clearInterval(interval); el.value = finalText; }
        iterations += 1 / 2;
    }, 30);
